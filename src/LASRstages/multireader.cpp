...
LASRmultireader::LASRmultireader(const LASRmultireader& other) : Stage(other)
{
...
bool LASRmultireader::set_chunk(Chunk& chunk)
{
  ...
  try
  {
    if (!build_sources(chunk)) return false;
  }
...
bool LASRmultireader::process(Header*& header)
{
  ...
  try
  {
    ...
    for (size_t i = 1 ; i < sources.size() ; i++)
    {
      ...
      {
        last_error = "sources with different scale or offset cannot be merged";
        return false;
      }
...
// Streaming mode
bool LASRmultireader::process(Point*& point)
{
  if (point == nullptr)
...
// In memory mode
bool LASRmultireader::process(PointCloud*& las)
{
...
void LASRmultireader::clear(bool)
{
  if (streaming && header)
  {
    delete header;
